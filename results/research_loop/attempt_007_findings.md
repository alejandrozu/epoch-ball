...
- Connectivity is not the full explanation. In the saved target-block connectivity audit, `5/10` ball blocks are already connected at `m=5` and `8/10` are connected at `m=6`, yet the continuity count is still zero, so the obstruction is the mismatch of local gluings and vertex images.
